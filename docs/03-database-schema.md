...
- **迁移清单**（0020-0032）：decrease_credits 用户级 advisory lock / refund_requested + credit_debts / 债务化准入 / 债务审计重键 / 资金 RPC 迁 private（0023）/ 全表 RLS deny-all（0024）/ 验证码列宽（0025）/ credit_lots 批次（0026）/ 默认管理员 pending_activation（0027）/ 联盟奖励冲销 `private.reverse_affiliate_reward`（0028，退款/拒付佣金 `completed→reversed`）/ 运营事件 outbox（0029，`private.op_event_outbox` + enqueue/claim/deliver/ack/fail/cleanup 六 RPC，warn+ 关键事件持久化重试）/ 管理员审批队列（0030，`private.admin_approvals`，N-6 双人复核：5 类高危动作审批单 + 发起人≠批准人 + 批准即执行/失败重试，RLS deny-all 仅授 service_role）/ 支付事件 inbox（0031，public `payment_events`：三渠道 webhook 先落库再处理，`UNIQUE(provider, provider_event_id)` 幂等，RLS deny-all 仅授 service_role）/ AI 请求状态机（0032，public `ai_requests`：`UNIQUE(user_uuid, request_id)` 按用户隔离幂等 + 崩溃补偿 + 24h TTL，RLS deny-all 仅授 service_role）/ 定价事务化批量写入（0033，`private.apply_payment_config(JSONB)` SECURITY DEFINER：审批快照原子写入 payment_products/payment_settings，先全量校验后写入，DB 层不变量再验，仅授 service_role）
- 0034：`orders.expired_at` 支付时刻重算 + public 残留副本 DROP
- 0035：GDPR 日志匿名化（`private.anonymize_user_personal_data(TEXT)` SECURITY DEFINER：op_events/outbox 队列/audit_logs 的 uuid 占位 + ip 抹除 + detail 脱敏，仅授 service_role）
- 0036：联盟奖励发放闭环（方案 A 自动转积分）：`private.handle_order_payment` 内 affiliates INSERT `RETURNING reward_amount` 后同事务发放邀请人奖励积分（`LEAST(订单积分×20%, max_reward/100)`、NULL 永久有效、`credit_lots` 批次同步 `source_type='affiliate_reward'`）；`private.reverse_affiliate_reward` 升级为翻状态 + 该订单奖励批次精确扣回 + `affiliate_reward` 负流水（签名/返回值不变，含过期批次照扣）
