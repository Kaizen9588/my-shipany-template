...
>
> ✅ **v1.5 整改关闭（2026-09-01，迁移 0032）**：第八轮指出的「缺幂等键、崩溃补偿、可恢复状态机」
> 已全部落地——`ai_requests` 表 + `UNIQUE(user_uuid, request_id)` 按用户隔离幂等 + cron 崩溃补偿
> + 输入硬限制（413/白名单 400）。设计正文见 §七；真实收费的该 No-Go 已解除。
>
> ✅ **P3-1（第九轮）已关闭**：整改设计正文已补为 §七 v1.5 落地注记；`docs/README` 的失效锚点
> 已改指本文 §七。
