...
> ⚠️ **P1-A（第十轮对抗式审查，2026-08-26）——trackServer 同样受 serverless 执行模型影响**：
> 「埋点失败静默」纪律成立的前提是埋点调用确实开始执行。Vercel serverless 响应返回后未 await 的 promise 不保证执行
> （全仓 `after()` / `waitUntil` 零命中），而漏斗 t3 锚点 `payment.succeeded` 恰恰是 webhook 服务端在响应前/后发出的事件——
> 若在响应后调度，t3 可能永久丢失，支付决策时长（t3-t2）与流失统计全部失真。
> **修法**：服务端埋点统一挂 Next.js `after()` 执行（与 docs/10 邮件、docs/16 告警同一处修法）。
