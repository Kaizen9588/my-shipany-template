...
| **管理员高风险操作必须审计持久化 + 最小权限** | ⚠️ 部分 | 退款、调账、定价、通知密钥、用户角色、封禁已有：最小权限（operator/admin/super_admin 分级）+ 服务端强制理由（N-6，`lib/admin-reason.ts`）+ reason 入 `audit_logs.detail`；`op_events` 持久化已闭合（N-4 outbox，0029）。仍缺：CSRF 防护、审批队列（N-6 剩余）。 |
